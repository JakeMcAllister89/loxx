...
export default function AcceptPlatformInvite() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase.functions.invoke("accept-platform-invite", { body: { action: "lookup", token } });
      if (error || !data?.invite) {
        setError("This invitation has expired or is no longer valid. Please contact LOXX to request a new invite.");
      } else {
        setInfo(data.invite);
        setFirstName(data.invite.first_name);
        setLastName(data.invite.last_name);
        setCompany(data.invite.company);
      }
      setLoading(false);
    })();
  }, [token]);
  ...
  const accept = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    const { data, error } = await supabase.functions.invoke("accept-platform-invite", {
      body: { action: "accept", token, first_name: firstName, last_name: lastName, company, password },
    });
    if (error || !data?.ok) { setBusy(false); toast.error((data as any)?.error ?? "Could not accept invitation"); return; }
          ...
          {loading ? (
            ...
            <>
              <h1 className="text-2xl font-semibold tracking-tight text-center">
                {firstName}, you've been invited to LOXX
              </h1>
              <p className="text-sm text-muted-foreground text-center mt-1">
                Set up your account to start managing your master key system.
              </p>
              <form onSubmit={accept} className="space-y-3 mt-6">
                ...
                <div><Label>Company</Label><Input value={company} onChange={e => setCompany(e.target.value)} required /></div>
